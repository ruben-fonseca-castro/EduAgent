"""HTML builder for generated lessons — renders Jinja2 template and saves to disk.

Adapted from friend_code/interactive_lessons/backend/utils/html_builder.py.
Uses app.config.settings instead of backend.config.get_settings().
"""
...
async def build_and_save_lesson(state: dict) -> Path:
    """Build HTML from lesson state and save to GENERATED_LESSONS_DIR.

    Returns the path to the saved HTML file.
    """
    lessons_dir = Path(settings.GENERATED_LESSONS_DIR)
    ...
    # Write metadata JSON (used by frontend to fetch lesson info)
    meta = {
        "lesson_id": lesson_id,
        "title": plan.get("title", "Lesson"),
